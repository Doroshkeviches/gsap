    ...
    return (
        <Canvas onClick={click} shadows dpr={[1, 1.5]} gl={{ antialias: false }} camera={{ position: [0, 0, 15], fov: 17.5, near: 1, far: 20 }} {...props}>
            ...
            <Physics /*debug*/ gravity={[0, 0, 0]}>
                <Pointer />
                {
                    connectors.map((props, i) =>  <Connector key={i} {...props} ></Connector>) /* prettier-ignore */
                }
...
function Model({ children, color = 'white', roughness = 0, ...props }) {
    ...
    return (
        <mesh ref={ref} castShadow receiveShadow scale={1} geometry={nodes.Object_2.geometry}>
            <meshStandardMaterial metalness={0.2} roughness={roughness} map={materials.defaultMat.map} />
            {children}
        </mesh>
    )
}
